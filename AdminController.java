...
public class AdminController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // --- API: VOUCHER DATA ---
        // Trong AdminController.java -> doGet -> if ("voucher_data".equals(action))
        if ("voucher_data".equals(action)) {
            ...
            // [MỚI] Truyền thêm startDate, endDate vào hàm đếm
            int totalVouchers = voucherDAO.getTotalVoucherCount(voucherSearch, startDate, endDate);

            int voucherTotalPages = (int) Math.ceil((double) totalVouchers / voucherPageSize);

            // [MỚI] Truyền thêm startDate, endDate vào hàm lấy danh sách
            List<Voucher> voucherList = voucherDAO.getPaginatedVouchers(voucherSearch, startDate, endDate, voucherPageIndex, voucherPageSize);
        ...
        try {
            if ("add_staff".equals(action)) {
                ...
                if (customerDAO.isUsernameTaken(username) || staffDAO.getStaffByEmailOrUsername(username) != null) {
                    ...
                    responseData.setIsSuccess(isSuccess);
                    if (!isSuccess) {
                        responseData.setDescription("Database error during insert.");
                    }
                }
