<%-- 
    ...
    Description: Staff Dashboard (Mirrored Admin Layout: Full Analytics)
--%>
...
<html lang="en">
    <head>
        ...
        <%-- ADDED JQUERY (Required for the sidebar code you requested) --%>
    ...
    <body id="staff-body">
        ...
        <%-- Include Staff Sidebar instead of Admin Sidebar --%>
        ...
        <script>
                            ...
                            // Access server-side data from StaffController
                            // Ensure to handle null/empty cases gracefully
                            let currentRevenueData = ${chartRevenue != null ? chartRevenue : '[]'};
                            let currentOrdersData = ${chartOrders != null ? chartOrders : '[]'};

                            document.addEventListener("DOMContentLoaded", function () {
                                // 1. Initialize Charts
                                initCharts();

                                // 2. Active Sidebar Item (Added Code)
                                // Ensure jQuery is loaded before running this
                                if (typeof $ !== 'undefined') {
                                    $('.nav-list li').removeClass('active');
                                    $('.nav-list li[data-target="dashboard"]').addClass('active');
                                } else {
                                    console.warn('jQuery is not loaded. Sidebar active state could not be set.');
                                }
                            });
                            ...
                            function updateChartData(year) {
                                // Note: Updated URL to point to staff controller
                                fetch('${BASE_URL}/staff?tab=api_chart_data&year=' + year)
