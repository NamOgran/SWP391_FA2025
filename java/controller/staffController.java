...
public class staffController extends HttpServlet {
    ...
    protected void addStaff(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        password = getMd5(password); // Đảm bảo phương thức getMd5() có sẵn
        String email = request.getParameter("email");
        String fullName = request.getParameter("fullName");
        String phone = request.getParameter("phone");
        String address = request.getParameter("address");
        ...
        // SỬA ĐỔI: Sử dụng constructor KHÔNG có staff_id
        staff s = new staff(username, email, password, address, phone, fullName, role);

        // Đảm bảo DAOstaff.signUp() cũng được cập nhật để không mong đợi staff_id trong câu lệnh INSERT
        boolean isSuccess = daoStaff.signUp(s);
        ResponseData data = new ResponseData();
        data.setIsSuccess(isSuccess);
        data.setData("");
        data.setDescription("");
...
//            request.setAttribute("productList", productList);
    ...
    protected void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String input = request.getParameter("input");   // username hoặc email
        String password = request.getParameter("password");
        password = getMd5(password); // Mã hóa password trước khi so sánh

        // Gọi DAO để lấy thông tin staff (bao gồm role)
