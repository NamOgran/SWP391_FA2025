<%-- 
    Document    : admin_header-sidebar
    Description : Reusable Header and Sidebar component for Admin pages
    Updated     : Added Dropdown Menu for Admin User
--%>
...
<%
    ...
    // Điều kiện: Nếu (s là null) HOẶC (role không phải là admin)
    // Lưu ý: "admin".equalsIgnoreCase giúp tránh lỗi NullPointerException và không phân biệt hoa thường
    if (s == null || !"admin".equalsIgnoreCase(s.getRole())) {
        
        // Xử lý điều hướng (Redirect)
        if (s == null) {
            // Trường hợp A: Chưa đăng nhập hoặc là Customer (không có session staff)
            // -> Đẩy về trang Login
            response.sendRedirect(request.getContextPath() + "/login.jsp");
        } else {
            // Trường hợp B: Đã đăng nhập là Staff nhưng KHÔNG PHẢI Admin
            // -> Đẩy về trang chủ (hoặc trang báo lỗi 403 tùy ý bạn)
            response.sendRedirect(request.getContextPath() + "/"); 
        }

        return; // [QUAN TRỌNG] Dừng việc tải trang Admin ngay lập tức
    }

    // --- Từ dòng này trở đi là khu vực an toàn chỉ dành cho Admin ---
%>
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Quicksand', sans-serif; }
    ...
    .admin-name { 
        ...
        padding: 10px; 
        border-radius: 4px;
        position: relative; /* Để căn chỉnh dropdown absolute */
    ...
    .admin-dropdown {
        ...
        position: absolute;
        top: 100%; /* Hiển thị ngay dưới tên */
        right: 0;
    ...
    /* Hiệu ứng hiển thị khi hover vào .admin-name */
    ...
    .nav-list li a i { min-width: 30px; font-size: 1.1em; text-align: left; }
        ...
        <ul class="nav-list">
            ...
            <li class="nav-link ${param.tab == 'personal' ? 'active' : ''}" data-target="personal-info">
                <a href="${BASE_URL}/admin?tab=personal">
                    <i class="bi bi-person-fill"></i> <span>Personal Information</span>
                </a>
            </li>
        </ul>
    </nav>
