...
<head>
    ...
    <style>
        ...
        /* ĐẢM BẢO 2 CỘT CANH TỪ TRÊN XUỐNG */
        .main-container .row.g-4 {
            align-items: flex-start;
        }

        /* BỌC DANH SÁCH CART ITEM – ÉP THÀNH CỘT DỌC */
        .cart-items-column {
            display: flex;
        ...
        .cart-item-card {
            ...
            padding: 20px;
            margin-bottom: 0; /* dùng gap ở .cart-items-column */
        ...
        /* ITEM HẾT HÀNG / KO CÒN TỒN TẠI */
        .cart-item-card.item-disabled {
            opacity: 0.6;
        }
        .cart-item-card.item-disabled .qty-btn {
            pointer-events: none;
            opacity: 0.4;
        }
        .cart-item-card.item-disabled .qty-control {
            cursor: not-allowed;
        }
                ...
                <div class="col-lg-8">
                    ...
                    <div class="cart-items-column">
                        <c:forEach items="${requestScope.cartList}" var="cart">
                            ...
                            <c:if test="${isMissingProduct or isOutOfStock or isInsufficient}">
                                <c:set var="hasCartError" value="true" />
                            </c:if>

                            <%-- 2. GIÁ LÚC ADD VÀ GIÁ HIỆN TẠI --%>
                            <%-- Giá lúc thêm vào cart (snapshot) --%>
                            <c:set var="priceAtAdd" value="${cart.price}" />

                            <%-- Giá hiện tại của sản phẩm (từ bảng product) --%>
                            <c:set var="priceNow" value="${priceP[cart.productID]}" />

                            <%-- Nếu map không có giá hiện tại thì coi như chưa đổi giá --%>
                            <c:if test="${empty priceNow}">
                                <c:set var="priceNow" value="${priceAtAdd}" />
                            </c:if>

                            <%-- Chỉ coi là thay đổi khi 2 giá khác nhau --%>
                            <c:set var="hasPriceChanged"
                                   value="${not empty priceNow and priceNow ne priceAtAdd}" />

                            <%-- Safe size để build id --%>
                            <c:set var="safeSize"
                                   value="${fn:replace(fn:replace(cart.size_name, ' ', '_'), '-', '_')}" />

                            <%-- Tổng tiền đang tính trong cart: dùng GIÁ LÚC ADD --%>
                            <c:set var="lineCurrent" value="${priceAtAdd * cart.quantity}" />
                            <fmt:formatNumber var="lineCurrentFmt" type="number"
                                              value="${lineCurrent}" pattern="###,###" />

                            <%-- Tổng tiền nếu tính theo GIÁ HIỆN TẠI (để show gạch ngang nếu đổi giá) --%>
                            <c:set var="lineOriginal" value="${priceNow * cart.quantity}" />
                            <fmt:formatNumber var="lineOriginalFmt" type="number"
                                              value="${lineOriginal}" pattern="###,###" />
                            <fmt:formatNumber var="unitPriceFmt" type="number"
                  value="${priceAtAdd}" pattern="###,###" />


                            <%-- 3. CARD SẢN PHẨM --%>
                            <div class="cart-item-card ${isMissingProduct or isOutOfStock ? 'item-disabled' : ''}"
...
<div class="product-meta">
    <%-- Lấy danh sách size khả dụng cho product này --%>
    ...
    <c:choose>
        <c:when test="${not empty sizesOfProduct}">
            <select class="size-select"
                    ...
                    <c:if test="${isMissingProduct or isOutOfStock}">disabled</c:if>>
                <c:forEach var="sz" items="${sizesOfProduct}">
                    <option value="${sz}"
                            <c:if test="${sz eq cart.size_name}">selected</c:if>>
                        ${sz}
                    </option>
                </c:forEach>
            </select>
        ...
        <c:otherwise>
            <span class="badge-size">${cart.size_name}</span>
        </c:otherwise>
    </c:choose>

    <%-- CẢNH BÁO TỒN KHO NHƯ CŨ --%>
    <c:choose>
        <c:when test="${isMissingProduct}">
            <div class="cart-warning error">
                This product is no longer available. Please remove it from your cart.
            </div>
        </c:when>
        <c:when test="${isOutOfStock}">
            <div class="cart-warning error">
                Out of stock. You can only remove this item.
            </div>
        </c:when>
        <c:when test="${isInsufficient}">
            <div class="cart-warning info">
                ...
                Please reduce the quantity before checkout.
                                    ...
                                    <div class="qty-control">
                                        <button type="button"
                                                class="qty-btn"
                                                onclick="decrementQuantity(${cart.productID},${cart.price},${cart.quantity}, '${cart.size_name}')"
                                                <c:if test="${isMissingProduct or isOutOfStock}">disabled</c:if>>
                                        ...
                                        <button type="button"
                                                class="qty-btn"
                                                onclick="incrementQuantity2(${cart.productID},${cart.price},${cart.quantity}, '${cart.size_name}')"
                                                <c:if test="${isMissingProduct or isOutOfStock}">disabled</c:if>>
                                            <i class="bi bi-plus"></i>
                                        </button>
                                    </div>
                                            <div class="unit-price">
    ${unitPriceFmt} VND / item
</div>

                                    <div class="cart-actions">
                                        <div class="price-display text-end"
                                             id="price2${cart.productID}${safeSize}">
                                            <%-- Giá đang dùng trong cart (giá snapshot) --%>
                                            <span class="current-price line-total">
                                                ${lineCurrentFmt} VND
                                            </span>

                                            <%-- Nếu giá hiện tại khác giá snapshot thì show thêm giá cũ + cảnh báo --%>
                                            <c:if test="${hasPriceChanged}">
                                                <div class="old-price"
                                                     id="price1${cart.productID}${safeSize}">
                                                    ${lineOriginalFmt} VND
                                                </div>
                                                <div class="cart-warning info">
                                                    Price of this item has changed since you added it to the cart.
                                                </div>
                                            </c:if>
                                        </div>
                    ...
                    <div class="summary-card">
                        ...
                        <form action="${pageContext.request.contextPath}/loadPayment" method="get">
                            <button class="checkout-btn"
                                    <c:if test="${hasCartError}">disabled</c:if>>
                                Proceed to Checkout
                            </button>
                        ...
                        <c:if test="${hasCartError}">
                            <div class="cart-warning error mt-2">
                                Some items are unavailable, out of stock, or exceed the remaining quantity.
                                Please fix or remove them before proceeding to checkout.
                            </div>
                        </c:if>
                    </div>
                </div>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (temp === 0) {
                    ...
                    holder.textContent =
                        (isNaN(line) || line <= 0) ? formatVND(price * newQty) : formatVND(line);
                    recalcTotals();
                } else {
...
function changeSize(productID, oldSize, selectEl) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (status === 'OK') {
                // cập nhật lại giỏ hàng để số lượng tồn, tổng tiền… đúng
                location.reload();
