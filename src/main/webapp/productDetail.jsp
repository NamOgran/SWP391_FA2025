F<%-- 
    ...
    Description : Product Detail - Updated logic for Int Discount Column
--%>
        ...
        <style>
            ...
            * {
            ...
            .size-option-radio:checked + .size-option-label {
                ...
                color: #fff;
                border-color: var(--primary-color);
                box-shadow: 0 5px 15px rgba(160, 129, 108, 0.4);
            ...
            .btn-buy-now:hover {
                ...
                transform: translateY(-4px);
                box-shadow: 0 12px 25px rgba(160, 129, 108, 0.4);
            }
            .btn-buy-now:active {
                transform: translateY(-1px);
            }
            ...
            .stock-popup.success::before {
                content: '\F26A';
                color: #2ecc71;
            }
            .stock-popup.error::before {
                content: '\F332';
                color: #e74c3c;
            }
            ...
            <c:if test="${empty ctx}"><c:set var="ctx" value=""/></c:if>
                ...
                <div class="breadcrumb-container reveal-element animate-up">
                    <nav aria-label="breadcrumb">
                        ...
                        <c:choose>
                            <c:when test="${ctx == 'male_tshirt'}"><li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/productList/male/t_shirt">T-shirt</a></li><li class="breadcrumb-separator"><i class="bi bi-chevron-right"></i></li></c:when>
                            <c:when test="${ctx == 'male_pant'}"><li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/productList/male/pant">Long Pants</a></li><li class="breadcrumb-separator"><i class="bi bi-chevron-right"></i></li></c:when>
                            <c:when test="${ctx == 'male_short'}"><li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/productList/male/short">Shorts</a></li><li class="breadcrumb-separator"><i class="bi bi-chevron-right"></i></li></c:when>
                            <c:when test="${ctx == 'female_tshirt'}"><li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/productList/female/t_shirt">T-shirt</a></li><li class="breadcrumb-separator"><i class="bi bi-chevron-right"></i></li></c:when>
                            <c:when test="${ctx == 'female_pant'}"><li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/productList/female/pant">Long Pants</a></li><li class="breadcrumb-separator"><i class="bi bi-chevron-right"></i></li></c:when>
                            <c:when test="${ctx == 'female_dress'}"><li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/productList/female/dress">Dresses</a></li><li class="breadcrumb-separator"><i class="bi bi-chevron-right"></i></li></c:when>
                            </c:choose>
                ...
                <div class="row align-items-start"> 
                    <div class="col-lg-6 mb-4 col-sticky-image reveal-element animate-scale">
                        <div class="product-image-container">
                            <img src="${p.picURL}" alt="${p.name}" onerror="this.src='${pageContext.request.contextPath}/images/default.jpg'">
                        </div>
                    </div>
                    ...
                    <div class="col-lg-6">
                        <div class="product-info-wrapper">
                            ...
                            <%-- ============================================================== --%>
                            <%-- [UPDATED] PRICE CALCULATION LOGIC USING INT DISCOUNT --%>
                            <%-- ============================================================== --%>

                            <%-- Lấy discount trực tiếp từ Product --%>
                            <c:set var="discountPercent" value="${p.discount}" />
                            <c:if test="${discountPercent == null}"> <c:set var="discountPercent" value="0"/> </c:if>

                            <%-- Tính giá gốc (format đẹp) --%>
                            <c:set var="formattedOriginalPrice"><fmt:formatNumber type="number" value="${p.price}" pattern="###,###" /></c:set>

                            <%-- Tính giá sau khi giảm --%>
                            <c:set var="finalPrice" value="${p.price}" />
                            <c:if test="${discountPercent > 0}">
                                <c:set var="finalPrice" value="${p.price - (p.price * discountPercent / 100)}" />
                            </c:if>
                            <c:set var="formattedFinalPrice"><fmt:formatNumber type="number" value="${finalPrice}" pattern="###,###" /></c:set>

                                <div class="price-wrapper reveal-element animate-up delay-200">
                                    <span class="current-price">${formattedFinalPrice} VND</span>

                                <%-- Hiển thị giá cũ và badge nếu có giảm giá --%>
                                <c:if test="${discountPercent > 0}">
                                    <span class="old-price">${formattedOriginalPrice} VND</span>
                            ...
                            <input type="hidden" name="price" class="price" value="${finalPrice}">
                            <input type="hidden" name="name" value="${p.name}">
                            <input type="hidden" name="picURL" value="${p.picURL}">
                            <input type="hidden" name="voucherPct" value="${discountPercent}">
                        </div>
                    </div>
                </div>
            </form>
        ...
        <script>
                                                            ...
                                                            document.addEventListener('DOMContentLoaded', function () {
                                                                ...
                                                                function checkStock() {
                                                                    ...
                                                                    if (stock <= 0) {
                                                                        showPopup("Size out of stock.");
                                                                        return false;
                                                                    }
                                                                    if (requested > stock) {
                                                                        showPopup(`Only ${stock} items left.`);
                                                                        return false;
                                                                    }
                                                                    return true;
                                                                }
