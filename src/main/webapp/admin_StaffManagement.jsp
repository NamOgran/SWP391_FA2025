<%-- 
    ...
    Description: Staff Management (UI Synced with Product & Customer Management)
--%>
...
<html lang="en">
    ...
    <head> 
        ...
        <style>
            ...
            /* [UPDATED] Validation Error Style - Matches Product/Customer Management */
            label.error { 
                color: #e74a3b; 
                font-size: 0.85rem; 
                margin-top: 5px; 
            ...
            .form-control.error { 
                ...
                background-color: #fff8f8 !important; /* Product Management Pink */
            }
    ...
    <body id="admin-body">
                            ...
                            <tbody id="staff-table-body">
                                <c:forEach var="staff" items="${staffList}">
                                    <tr class="staff-row">
                                        ...
                                        <td>
                                            <div class="cell-truncate text-muted small" title="${staff.address}">
                                                ${staff.address}
                                            </div>
                                        </td>
                ...
                <div class="modal-content modal-content-modern">
                    ...
                    <div class="modal-body p-4">
                        <form action="${BASE_URL}/admin" method="POST" id="updateAccountForm">
                            ...
                            <input type="hidden" id="update_acc_type" name="type" value="staff"> 
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="input-group">
                                        ...
                                        <div class="form-floating">
                                            <input type="text" class="form-control bg-light" id="update_acc_username" name="username" readonly>
                                ...
                                <div class="col-md-6">
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="bi bi-card-text"></i></span>
                                        <div class="form-floating">
                                            <input type="text" class="form-control" id="update_acc_fullName" name="fullName" required maxlength="50" placeholder="Name">
                                            <label>Full Name</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="bi bi-envelope"></i></span>
                                        <div class="form-floating">
                                            <input type="email" class="form-control" id="update_acc_email" name="email" required maxlength="100" placeholder="Email">
                                            <label>Email</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="bi bi-telephone"></i></span>
                                        <div class="form-floating">
                                            <input type="text" class="form-control" id="update_acc_phone" name="phone" required placeholder="Phone">
                                            <label>Phone</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="bi bi-geo-alt"></i></span>
                                        <div class="form-floating">
                                            <input type="text" class="form-control" id="update_acc_address" name="address" required maxlength="255" placeholder="Address">
                                            <label>Address</label>
                                        </div>
                                    </div>
                                </div>
        ...
        <script>
            ...
            // Cấu hình chung cho Validation giống Product/Customer Management
            ...
            function renderStaffPage(page) {
                currentPage = page;
                allStaffData.forEach(item => { item.row.style.display = 'none'; });
                ...
                pageItems.forEach(item => {
                    item.row.style.display = 'table-row';
                ...
                if(total > 1) {
                    if(page > 1) html += `<li class="custom-page-item"><a class="custom-page-link" onclick="renderStaffPage(\${page-1})"><i class="bi bi-chevron-left"></i></a></li>`;
                    for(let i=1; i<=total; i++) {
                        if (i === 1 || i === total || (i >= page - 1 && i <= page + 1)) {
                            html += `<li class="custom-page-item \${i===currentPage?'active':''}"><a class="custom-page-link" onclick="renderStaffPage(\${i})">\${i}</a></li>`;
                        } else if (i === page - 2 || i === page + 2) {
                            html += `<li class="custom-page-item"><span class="custom-page-link border-0">...</span></li>`;
                        }
                    }
                    if(page < total) html += `<li class="custom-page-item"><a class="custom-page-link" onclick="renderStaffPage(\${page+1})"><i class="bi bi-chevron-right"></i></a></li>`;
                }
                html += '</ul>';
            ...
            if (updateAccountModal) {
                updateAccountModal.addEventListener('show.bs.modal', function (event) {
                    var button = event.relatedTarget;
                    $('#update_acc_id').val(button.getAttribute('data-id'));
                    $('#update_acc_username').val(button.getAttribute('data-username'));
                    $('#update_acc_email').val(button.getAttribute('data-email'));
                    $('#update_acc_fullName').val(button.getAttribute('data-fullname'));
                    $('#update_acc_phone').val(button.getAttribute('data-phone'));
                    $('#update_acc_address').val(button.getAttribute('data-address'));
                });
            }
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.isSuccess) { showToast('Staff added successfully!'); setTimeout(()=>location.reload(), 1000); }
                            else {
            ...
            $("#updateAccountForm").validate({
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url: $(form).attr('action'), type: 'POST', data: $(form).serialize(), dataType: 'json',
                        success: function (data) {
                            if (data.isSuccess) { showToast('Staff updated!'); setTimeout(()=>location.reload(), 1000); }
                            else { showToast(data.description, 'error'); $btn.prop('disabled', false).html('Save Changes'); }
                        }
                    });
                }
            });
            ...
            $(confirmDeleteButton).on('click', function () {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function (data) {
                        if (data.isSuccess) { showToast('Staff deleted'); setTimeout(()=>location.reload(), 1000); }
                        else { showToast(data.description, 'error'); btn.prop('disabled', false).html('Confirm Delete'); }
                    }
                });
            });
        </script>
