<%--
    ...
    Updated     : Logic hiển thị ảnh (Local Project + External URL)
--%>
                            ...
                            <div class="product-card">
                                <div class="product-card__image-wrapper">
                                    ...
                                    <a href="${pageContext.request.contextPath}/productDetail?id=${product.id}">
                                        ...
                                        <%-- ========================================================= --%>
                                        <%-- LOGIC XỬ LÝ ẢNH (LOCAL PROJECT FOLDER hoặc EXTERNAL URL) --%>
                                        <%-- ========================================================= --%>
                                        <c:choose>
                                            <%-- 1. Nếu ảnh là URL online (bắt đầu bằng http) --%>
                                            <c:when test="${fn:startsWith(product.picURL, 'http')}">
                                                <c:set var="imgSrc" value="${product.picURL}" />
                                            </c:when>
                                            
                                            <%-- 2. Nếu ảnh là Local File --%>
                                            <c:otherwise>
                                                <%-- Mặc định thêm context path vào trước --%>
                                                <c:set var="imgSrc" value="${pageContext.request.contextPath}/${product.picURL}" />
                                                
                                                <%-- Tuy nhiên, nếu trong DB đã lưu cứng /ProjectName/... (dữ liệu cũ) thì dùng luôn --%>
                                                <c:if test="${fn:startsWith(product.picURL, pageContext.request.contextPath)}">
                                                    <c:set var="imgSrc" value="${product.picURL}" />
                                                </c:if>
                                            </c:otherwise>
                                        </c:choose>
