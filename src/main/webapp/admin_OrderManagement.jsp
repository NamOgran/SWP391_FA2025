<%-- 
    Document   : admin_OrderManagement.jsp
    Description: Order Management (Updated: Shipping Fee Logic, FullName, No Processing)
--%>
...
<head>
    ...
    <style>
        ...
        .table-modern tr.selected-row td { background-color: #eef2ff !important; border-top: 2px solid #4e73df; border-bottom: none; }
        .table-modern tr.selected-row td:first-child { border-left: 2px solid #4e73df; }
        .table-modern tr.selected-row td:last-child { border-right: 2px solid #4e73df; }
        ...
        .btn-soft-danger { background: rgba(231, 74, 59, 0.15); color: #e74a3b; }
        .btn-soft-danger:hover { background: #e74a3b; color: #fff; }
        
        .btn-soft-info { background: rgba(54, 185, 204, 0.15); color: #36b9cc; }
...
<body id="admin-body">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <div class="input-group search-input-group">
                            ...
                            <input type="text" id="filterSearch" class="form-control" placeholder="Order ID, Customer Name, Address...">
                        </div>
                    </div>
                    <div class="col-md-2">
                        ...
                        <tbody id="order-table-body">
                            <c:forEach items="${orderList}" var="order" varStatus="loop">
                                <tr class="order-summary-row">
                                    ...
                                    <td>
                                        <div class="user-cell">
                                            <%-- [FIX] Display Customer Full Name from Map --%>
                                    ...
                                    <td class="text-end fw-bold text-dark">
                                        <%-- Note: Value here already includes shipping fee if calculated in Controller --%>
                                        <fmt:formatNumber value="${calculatedTotalMap[order.orderID]}" pattern="###,###" /> <small class="text-muted">VND</small>
                                    </td>
                                    <td class="text-center">
                                        <%-- [FIX] Logic: Only Pending allowed, Processing Removed --%>
                                        <c:if test="${order.status eq 'Pending'}">
                                            <button class="btn-soft btn-soft-success" onclick="updateStatus(${order.orderID}, 'Delivering')" title="Approve/Deliver">
                                                <i class="bi bi-truck"></i>
                                            </button>
                                            <button class="btn-soft btn-soft-danger" onclick="updateStatus(${order.orderID}, 'Cancelled')" title="Reject">
                                                <i class="bi bi-x-lg"></i>
                                            </button>
                                        </c:if>
                                        
                                        <c:if test="${order.status eq 'Delivering'}">
                                            <button class="btn-soft btn-soft-success" onclick="updateStatus(${order.orderID}, 'Delivered')" title="Mark as Delivered">
                                                    ...
                                                    <table class="table table-sm table-striped mb-0">
                                                        ...
                                                        <tbody>
                                                            <c:forEach items="${orderDetailList}" var="d">
                                                                <%-- Kiểm tra ID để hiển thị đúng sản phẩm --%>
                                                                <c:if test="${d.orderID eq order.orderID}">
                                                                    <tr>
                                                                        <td class="ps-3 align-middle">
                                                                            <img src="${picUrlMap[d.productID]}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px; border: 1px solid #ddd;" onerror="this.src='https://via.placeholder.com/40'">
                                                                        </td>
                                                                        ...
                                                                        <td class="text-end align-middle pe-3">
                                                                            ...
                                                                            <c:set var="pVoucher" value="${voucherMap[productVoucherMap[d.productID]]}" />
                                                                            <c:set var="finalP" value="${pPrice - (pPrice * (pVoucher!=null?pVoucher:0)/100)}" />
    ...
    <script>
        ...
        function updateStatus(id, status) {
            ...
            $.post('${BASE_URL}/admin', { action: 'update_order_status', orderId: id, status: status }, function(res) {
                 if(res.isSuccess) location.reload(); else alert(res.description);
            }, 'json');
        }
        ...
        function initPagination() {
            ...
            rows.forEach(r => {
                ...
                // Parse dd/MM/yyyy to yyyy-MM-dd for HTML date input comparison
        ...
        function renderPage(page) {
            ...
            filteredRows.slice(start, end).forEach(item => item.el.style.display = 'table-row');
