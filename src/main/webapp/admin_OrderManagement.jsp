<%-- 
    ...
    Description: Order Management (Updated: Database Total Amount, Removed Logic Calculation)
--%>
...
<head>
    ...
    <style>
        ...
        .detail-row-container { 
            background-color: #ebecf0; padding: 25px; 
            box-shadow: inset 0 0 15px rgba(0,0,0,0.1); 
            border-bottom: 1px solid #e3e6f0;
        }
        .detail-card { 
            background: #fff; border-radius: 12px; 
            border: 1px solid #d1d3e2; box-shadow: 0 5px 15px rgba(0,0,0,0.05); 
            overflow: hidden; 
        }
        .detail-header { 
            background: linear-gradient(45deg, #4e73df, #224abe); color: white; 
            padding: 12px 20px; font-weight: 600; display: flex; align-items: center; gap: 10px; 
        }
                    ...
                    <table class="table table-modern align-middle" id="orderTableMain">
                        <thead>
                            <tr>
                                ...
                                <%-- [CHANGED] Header from Total Price to Total Amount --%>
                        ...
                        <tbody id="order-table-body">
                            <c:forEach items="${orderList}" var="order" varStatus="loop">
                                <%-- [REMOVED] Logic calculation variables (finalTotal, subTotal, shipFee) --%>
                                ...
                                <tr class="order-summary-row" id="row-${order.orderID}">
                                    ...
                                    <%-- [UPDATED] Use direct database value ${order.totalAmount} --%>
                                                        ...
                                                        <tbody>
                                                            <c:forEach items="${orderDetailList}" var="d">
                                                                <c:if test="${d.orderID eq order.orderID}">
                                                                    ...
                                                                    <%-- CALCULATE UNIT PRICE & DISCOUNT (For display only, not for order total) --%>
                                                            ...
                                                            <%-- [REMOVED] Subtotal, Shipping Fee, and Total rows here --%>
