<%-- 
    ...
    Description : Import Management (Full Filters + AJAX Reload + Validations + Cancel Action)
--%>
...
<head>
    ...
    <style>
        ...
        .btn-soft-secondary { background: rgba(133, 135, 150, 0.15); color: #858796; } .btn-soft-secondary:hover { background: #858796; color: #fff; }
...
<body id="admin-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group search-input-group">
                            ...
                            <input type="text" id="filterSearch" class="form-control" placeholder="Search ID, Username...">
                        </div>
                    </div>
                    <%-- Date --%>
                    <div class="col-md-2">
                                ...
                                <c:when test="${not empty importList}">
                                    <c:forEach items="${importList}" var="imp">
                                        <tr class="import-summary-row">
                                            <td class="fw-bold text-primary">#${imp.id}</td>
                                            <td>
                                                <div class="user-cell">
                                            ...
                                            <td class="text-end fw-bold text-primary import-price">
                                            ...
                                            <td class="text-center">
                                                <div class="d-inline-flex">
                                                    <c:if test="${imp.status eq 'Pending'}">
                                                        <%-- Confirm Button --%>
                                                        <button class="btn-soft btn-soft-success me-1 confirm-btn" onclick="confirmImport(${imp.id})" title="Confirm Import">
                                                            <i class="bi bi-check-lg"></i>
                                                        </button>
                                                        
                                                        <%-- Cancel Button [NEW] --%>
                                                        <button class="btn-soft btn-soft-danger me-1 cancel-btn" onclick="cancelImport(${imp.id})" title="Cancel Import">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            setTimeout(() => {
                $('#table-loader').fadeOut(200, () => { 
                    ...
                    initPagination(); // Initialize data
                });
            }, 400);
        ...
        function initPagination() {
            ...
            initAvatars();
            applyFilters(); // Re-apply existing filters if any
        }
        ...
        function renderPage(page) {
            ...
            allRowsData.forEach(item => { 
                ...
                item.el.classList.remove('table-active'); 
        ...
        function refreshTableData() {
            ...
            $.get(window.location.href, function(response) {
                // Parse the response to extract the new table body
        ...
        function renderImportTable() {
            ...
            if (importItems.length === 0) {
                ...
                importItems.forEach((item, i) => {
                    // Added type="button" to remove button
                    tbody.innerHTML += `
                        <tr>
        ...
        function submitImport() {
            ...
            $.post('${BASE_URL}/admin', { action: 'create_import', items: JSON.stringify(importItems) }, function(res){
                if(res.isSuccess) {
                    ...
                    resetModal();
                    // AJAX Reload instead of page reload
                    refreshTableData();
        ...
        function cancelImport(id) {
            if (!confirm("Are you sure you want to CANCEL this import order? This action cannot be undone.")) return;
            
            toggleTableBlur(true);
            
            // Gửi request về AdminController
            $.post('${BASE_URL}/admin', { action: 'cancel_import', id: id }, function(res){
                if(res.isSuccess) { 
                    showToast("Import order cancelled successfully!");
                    // Reload lại bảng dữ liệu bằng AJAX
                    refreshTableData();
                } else { 
                    showToast(res.description || "Could not cancel order.", "error");
                    toggleTableBlur(false);
                }
            }, 'json')
            .fail(function() { 
