<%-- 
    ...
    Description: Product Management (Fixed: Binary Upload, Custom Validation, Layout)
--%>
...
<html lang="en">
    ...
    <head> 
        ...
        <style>
            ...
            .product-img-thumb {
                width: 60px; height: 60px; object-fit: cover; border-radius: 8px; border: 1px solid #eee;
            }
            ...
            .btn-soft-warning { background: rgba(246, 194, 62, 0.1); color: #f6c23e; }
            ...
            .modal-header-modern h5 { margin: 0; font-weight: 700; }
            ...
            .img-preview-box {
                ...
                background: #f8f9fa; position: relative; margin-bottom: 15px;
            }
            ...
            .row-inactive .badge-inactive {
                opacity: 1; 
            }
            .row-inactive .btn-soft {
                filter: grayscale(100%);
            }
            .row-inactive .btn-soft:hover {
                filter: none;
            }
    ...
    <body id="admin-body">
        ...
        <c:if test="${not empty param.msg}">
            <div id="toast-message" class="toast-notification active 
                 ${fn:contains(param.msg, 'error') || param.msg == 'deleted' ? 'toast-danger' : 
                   (param.msg == 'deactivated' ? 'toast-warning' : 
                   (param.msg == 'updated' ? 'toast-info' : 'toast-success'))}">
                <i class="fas ${fn:contains(param.msg, 'error') || param.msg == 'deleted' ? 'fa-exclamation-circle' : 
                              (param.msg == 'deactivated' ? 'fa-eye-slash' : 
                              (param.msg == 'updated' ? 'fa-info-circle' : 'fa-check-circle'))}"></i>
                <span id="toast-text">
                    <c:choose>
                        ...
                        <c:when test="${param.msg == 'activated'}">Product reactivated!</c:when>
                        <c:when test="${param.msg == 'deactivated'}">Product deactivated!</c:when>
            ...
            <div class="card-modern">
                <div class="card-header-modern">
                    <div class="page-title">
                        ...
                        <span class="stat-badge"><c:out value="${totalProducts}" default="${fn:length(list)}"/> Products</span>
                    </div>
                ...
                <div class="filter-container">
                    <form action="${BASE_URL}/admin" method="get" class="row g-2 align-items-center">
                        <input type="hidden" name="tab" value="product">
                        
                        <div class="col-md-3">
                            <div class="input-group search-input-group">
                                <span class="input-group-text"><i class="bi bi-search"></i></span>
                                <input type="text" name="search" class="form-control" placeholder="Search by Name..." value="${searchName}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-1">
                             <a href="${BASE_URL}/admin?tab=product" class="btn btn-light w-100 border" title="Reset Filters"><i class="bi bi-arrow-counterclockwise"></i></a>
                        </div>
                    </form>
                </div>
    ...
    <div class="d-flex flex-column align-items-center gap-2">
        ...
        <div class="d-flex gap-2">
            <button type="button" class="btn-soft btn-soft-primary" data-bs-toggle="modal" data-bs-target="#updateProductModal"
                    ...
                    data-categoryid="${p.categoryID}" data-voucherid="${p.voucherID}" title="Edit">
            ...
            <c:choose>
                <c:when test="${p.is_active}">
                    <button type="button" class="btn-soft btn-soft-warning" data-bs-toggle="modal" data-bs-target="#toggleStatusModal"
                            ...
                            data-product-name="${p.name}" data-action-type="Deactivate" title="Deactivate">
                ...
                <c:otherwise>
                    <button type="button" class="btn-soft btn-soft-success" data-bs-toggle="modal" data-bs-target="#toggleStatusModal"
                            data-action-url="${BASE_URL}/toggleProductStatus?id=${p.id}&page=${currentPage}&sort=${sortBy}&search=${encodedSearch}&category=${selectedCategory}&status=${selectedStatus}&tab=product"
                            data-product-name="${p.name}" data-action-type="Reactivate" title="Reactivate">
                        ...
                        <div class="pagination-wrapper px-4 pb-3">
                            <ul class="custom-pagination">
                                <li class="custom-page-item ${currentPage == 1 ? 'disabled' : ''}">
                                    <a class="custom-page-link" href="${BASE_URL}/admin?page=${currentPage - 1}&sort=${sortBy}&search=${encodedSearch}&category=${selectedCategory}&status=${selectedStatus}&tab=product"><i class="bi bi-chevron-left"></i></a>
                                </li>
                                <c:forEach begin="1" end="${totalPages}" var="i">
                                    <li class="custom-page-item ${i == currentPage ? 'active' : ''}">
                                        <a class="custom-page-link" href="${BASE_URL}/admin?page=${i}&sort=${sortBy}&search=${encodedSearch}&category=${selectedCategory}&status=${selectedStatus}&tab=product">${i}</a>
                                    </li>
                                </c:forEach>
                                <li class="custom-page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                    <a class="custom-page-link" href="${BASE_URL}/admin?page=${currentPage + 1}&sort=${sortBy}&search=${encodedSearch}&category=${selectedCategory}&status=${selectedStatus}&tab=product"><i class="bi bi-chevron-right"></i></a>
                                </li>
                            </ul>
                        </div>
                    </c:if>
                    ...
                    <div class="modal-body">
                        ...
                        <div id="relatedDataContent" style="display: none;">
                            ...
                            <div class="tab-content" id="relatedDataTabContent">
                                <div class="tab-pane fade show active" id="sizes-content"><p id="sizes-none" class="text-muted small text-center py-3" style="display: none;">No data found.</p><table class="table table-sm table-bordered"><thead class="table-light"><tr><th>Size Name</th><th>Quantity</th></tr></thead><tbody id="sizes-table-body"></tbody></table></div>
                                <div class="tab-pane fade" id="carts-content"><p id="carts-none" class="text-muted small text-center py-3" style="display: none;">No data found.</p><table class="table table-sm table-bordered"><thead class="table-light"><tr><th>Cart ID</th><th>Customer ID</th><th>Size</th><th>Quantity</th></tr></thead><tbody id="carts-table-body"></tbody></table></div>
                                <div class="tab-pane fade" id="orders-content"><p id="orders-none" class="text-muted small text-center py-3" style="display: none;">No data found.</p><table class="table table-sm table-bordered"><thead class="table-light"><tr><th>Order ID</th><th>Size</th><th>Quantity</th></tr></thead><tbody id="orders-table-body"></tbody></table></div>
                                <div class="tab-pane fade" id="imports-content"><p id="imports-none" class="text-muted small text-center py-3" style="display: none;">No data found.</p><table class="table table-sm table-bordered"><thead class="table-light"><tr><th>Import ID</th><th>Date</th><th>Total</th><th>Status</th></tr></thead><tbody id="imports-table-body"></tbody></table></div>
                                <div class="tab-pane fade" id="feedbacks-content"><p id="feedbacks-none" class="text-muted small text-center py-3" style="display: none;">No data found.</p><table class="table table-sm table-bordered"><thead class="table-light"><tr><th>ID</th><th>Customer</th><th>Rating</th></tr></thead><tbody id="feedbacks-table-body"></tbody></table></div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer justify-content-center border-0">
                        ...
                        <a id="confirmToggleButton" href="#" class="btn btn-primary px-4">Confirm</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            function handleFileUpload(input, urlInputId, imgId, boxId) {
                if (input.files && input.files[0]) {
                    ...
                    // Create Blob URL for preview (Very fast, no base64 overhead)
                    var objectURL = URL.createObjectURL(file);
                    
                    $('#' + imgId).attr('src', objectURL).show();
                    $('#' + boxId + ' .img-placeholder').hide();
                    
                    // Optional: Show filename in text input for UX, though backend uses file part
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                setTimeout(() => {
                    $('#table-loader').fadeOut(200, () => { $('#product-content-container').fadeIn(200); });
                }, 400);
            });
            ...
            $("#addForm").validate({
                rules: { 
                    ...
                    price: {required: true, number: true, min: 1, max: 999999999}, 
                    category: {required: true}, 
                    des: {required: true, maxlength: 255}, 
                    // Validate: Require pic URL text OR file input
                    pic: { required: function(element) { return $('#add_file_input')[0].files.length === 0; } } 
                },
                messages: {
                    price: {
                        max: "Max Price is 999.999.999"
                    }
                },
                errorElement: "label", errorClass: "error",
                errorPlacement: function (error, element) { error.insertAfter(element.closest('.form-floating') || element); },
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        type: 'POST', 
                        data: formData,
                        processData: false, // Important for binary upload
                        contentType: false, // Important for binary upload
                        dataType: 'json',
                        success: function (data) {
                            if (data.isSuccess) window.location.href = window.location.href.split('?')[0] + '?tab=product&msg=added';
                            else { alert(data.description); $btn.prop('disabled', false).text('Add Product'); }
                        },
            ...
            $("#updateForm").validate({
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url: $(form).attr('action'), type: 'POST', data: formData, processData: false, contentType: false, dataType: 'json',
                        success: function (data) {
                            if (data.isSuccess) window.location.href = window.location.href.split('?')[0] + '?tab=product&msg=updated';
                            else { alert(data.description); $btn.prop('disabled', false).text('Save Changes'); }
                        },
            ...
            if (updateModal) {
                updateModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    $('#update_voucher').val(btn.getAttribute('data-voucherid'));
            ...
            if (deleteConfirmationModal) {
                deleteConfirmationModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    $.ajax({
                        url: '${BASE_URL}/admin/productRelatedData', type: 'GET', data: {productId: productId}, dataType: 'json',
                        success: function (data) {
                            var reasons = [];
                            var canDelete = true;
                            ...
                            if (data.sizes && data.sizes.length > 0) {
                                ...
                                if (totalStock > 0) {
                                    ...
                                    $('#sizes-count').text(data.sizes.length).removeClass('bg-secondary').addClass('bg-success');
                                }
                            } else { 
                            ...
                            if (data.carts && data.carts.length > 0) { 
                                ...
                                canDelete = false;
                            } else { $('#carts-count').text(0).addClass('bg-secondary'); $('#carts-none').show(); }

                            // Check Orders
                            if (data.orders && data.orders.length > 0) { 
                                $('#orders-count').text(data.orders.length).removeClass('bg-secondary').addClass('bg-warning'); 
                                ...
                                reasons.push("Product exists in " + data.orders.length + " orders.");
                                canDelete = false;
                            } else { $('#orders-count').text(0).addClass('bg-secondary'); $('#orders-none').show(); }

                            // Check Imports
                            if (data.imports && data.imports.length > 0) { 
                                $('#imports-count').text(data.imports.length).removeClass('bg-secondary').addClass('bg-warning'); 
                                ...
                                reasons.push("Product has import history.");
                                canDelete = false;
                            } else { $('#imports-count').text(0).addClass('bg-secondary'); $('#imports-none').show(); }

                            // Check Feedbacks
                            if (data.feedbacks && data.feedbacks.length > 0) { 
                                $('#feedbacks-count').text(data.feedbacks.length).removeClass('bg-secondary').addClass('bg-warning'); 
                                ...
                                reasons.push("Product has user feedbacks.");
                                canDelete = false;
                            } else { $('#feedbacks-count').text(0).addClass('bg-secondary'); $('#feedbacks-none').show(); }
                            ...
                            var alertHtml = '';
                            if (!canDelete) {
                                alertHtml = '<div class="alert alert-danger"><strong>Cannot Delete:</strong><ul class="mb-0 ps-3">';
                                reasons.forEach(function(r) { alertHtml += '<li>' + r + '</li>'; });
                                alertHtml += '</ul></div>';
                                $('#confirmDeleteButton').hide();
                            } else {
                                alertHtml = '<div class="alert alert-success"><strong>Safe to Delete:</strong> Product has no active stock or dependencies.</div>';
                                $('#confirmDeleteButton').show();
                            }
                            $('#deleteStatusAlert').html(alertHtml);
                        },
            ...
            $('#confirmDeleteButton').off('click').on('click', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.isSuccess) { window.location.href = window.location.href.split('?')[0] + '?tab=product&msg=deleted'; } 
                        else { alert('Error: ' + (data.description || 'An unknown error occurred.')); btn.prop('disabled', false).html('<i class="bi bi-trash-fill me-2"></i> Confirm Delete'); }
                    },
                    error: function () { alert('Server error. Could not delete product.'); btn.prop('disabled', false).html('<i class="bi bi-trash-fill me-2"></i> Confirm Delete'); }
                });
            });
            ...
            if (toggleModal) {
                toggleModal.addEventListener('show.bs.modal', function (event) {
                    const btn = event.relatedTarget;
                    $('#confirmToggleButton').attr('href', btn.getAttribute('data-action-url'));
                    if (btn.getAttribute('data-action-type') === 'Deactivate') {
                        $('#confirmToggleButton').removeClass('btn-primary').addClass('btn-warning');
                    } else {
                        $('#confirmToggleButton').removeClass('btn-warning').addClass('btn-success');
                    }
                });
            }
        </script>
