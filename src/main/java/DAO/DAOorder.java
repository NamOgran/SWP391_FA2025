...
public class DAOorder extends DBconnect.DBconnect {
        ...
        try {
            ...
            while (rs.next()) {
                orders o = new orders(rs.getInt("order_id"), rs.getString("address"), rs.getDate("date"), rs.getString("status"),
                        rs.getString("phone_number"), rs.getInt("customer_id"), rs.getInt("staff_id"), rs.getInt("total"));
                list.add(o);
            }
        } catch (Exception e) {
    ...
    public void insertOrder(String address, Date date, String status, String phoneNumber, int customer_id, int staff_id, float total) {
