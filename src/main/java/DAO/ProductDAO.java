...
public class ProductDAO extends DBConnect.DBConnect {
    ...
    // Helper: Map dữ liệu từ ResultSet vào Entity Product mới
    private Product mapRow(ResultSet rs) throws SQLException {
        return new Product(
                rs.getInt("product_id"),
                rs.getInt("price"),
                rs.getInt("category_id"),
                rs.getInt("promo_id"), // [FIX] DB uses 'promo_id' instead of 'voucher_id'
                rs.getString("name"),
    ...
    public boolean insert(Product p) {
        // [FIX] Changed 'voucher_id' to 'promo_id'
        String sql = "INSERT INTO product(name, description, pic_url, price, category_id, promo_id, is_active) VALUES (?,?,?,?,?,?,?)";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
            ...
            st.setInt(6, p.getVoucherID()); // Java Entity still uses getVoucherID()
    ...
    public boolean update(Product p) {
        // [FIX] Changed 'voucher_id' to 'promo_id'
        String sql = "UPDATE product SET name=?, description=?, pic_url=?, price=?, category_id=?, promo_id=?, is_active=? WHERE product_id=?";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
            st.setString(1, p.getName());
            st.setString(2, p.getDescription());
            st.setString(3, p.getPicURL());
    ...
    // --- CÁC HÀM TÌM KIẾM & LỌC (DÙNG CHO ADMIN) ---
    ...
    // --- CÁC HÀM HIỂN THỊ TRANG CHỦ (SHOPPING) ---
    ...
    public boolean hasDataForVoucher(int voucherId) {
        // [FIX] Changed 'voucher_id' to 'promo_id'
    ...
    public List<Product> getProductsByVoucherId(int voucherId) {
        List<Product> list = new ArrayList<>();
        // [FIX] Changed 'voucher_id' to 'promo_id'
    ...
    // Các hàm thống kê (Giữ nguyên logic SQL cũ)
