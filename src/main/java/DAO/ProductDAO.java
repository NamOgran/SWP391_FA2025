...
public class ProductDAO extends DBConnect.DBConnect {
    ...
    // [CẬP NHẬT] MapRow: Lấy cột 'discount' (int)
    private Product mapRow(ResultSet rs) throws SQLException {
        return new Product(
                rs.getInt("product_id"),
                rs.getInt("price"),
                rs.getInt("category_id"),
                rs.getInt("discount"), // [SỬA] Lấy int discount
                rs.getString("name"),
    ...
    public boolean insert(Product p) {
        // [CẬP NHẬT] SQL dùng cột discount
        String sql = "INSERT INTO product(name, description, pic_url, price, category_id, discount, is_active) VALUES (?,?,?,?,?,?,?)";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
    ...
    public boolean update(Product p) {
        // [CẬP NHẬT] SQL dùng cột discount
        String sql = "UPDATE product SET name=?, description=?, pic_url=?, price=?, category_id=?, discount=?, is_active=? WHERE product_id=?";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
            st.setString(1, p.getName());
            st.setString(2, p.getDescription());
            st.setString(3, p.getPicURL());
    ...
    public String deleteProductWithChecks(int productId) {
        try {
            ...
            if (size_detailDAO.hasDataForProduct(productId)) {
                return "Deletion failed: Has related data in Size_detail.";
            }

            CartDAO cartDAO = new CartDAO();
            if (cartDAO.hasDataForProduct(productId)) {
                return "Deletion failed: Has related data in Cart.";
            }

            OrderDAO orderDAO = new OrderDAO();
            if (orderDAO.hasDataForProduct(productId)) {
                return "Deletion failed: Has related data in Orders.";
            }

            ImportDetailDAO importDetailDAO = new ImportDetailDAO();
            if (importDetailDAO.hasDataForProduct(productId)) {
                return "Deletion failed: Has related data in Imports.";
            }

            FeedBackDAO feedBackDAO = new FeedBackDAO();
            if (feedBackDAO.hasDataForProduct(productId)) {
                return "Deletion failed: Has related data in Feedback.";
            }
        } catch (Exception e) {
    ...
    public List<Product> getPublicProducts(String context, String sort1, String sort2, String priceRange, String categoryFilter) {
        ...
        if (context != null && !context.isEmpty()) {
            if (context.contains("female")) {
                if (context.contains("tshirt")) {
                    sql.append(" AND p.category_id = ").append(catDao.getIdType("t-shirt", "female"));
                } else if (context.contains("dress")) {
                    sql.append(" AND p.category_id = ").append(catDao.getIdType("dress", "female"));
                } else if (context.contains("pant")) {
                    sql.append(" AND p.category_id = ").append(catDao.getIdType("pant", "female"));
                } else {
                    sql.append(" AND p.category_id IN ").append(catDao.getIdGender("female"));
                }
            } else if (context.contains("male")) {
                if (context.contains("tshirt")) {
                    sql.append(" AND p.category_id = ").append(catDao.getIdType("t-shirt", "male"));
                } else if (context.contains("short")) {
                    sql.append(" AND p.category_id = ").append(catDao.getIdType("short", "male"));
                } else if (context.contains("pant")) {
                    sql.append(" AND p.category_id = ").append(catDao.getIdType("pant", "male"));
                } else {
                    sql.append(" AND p.category_id IN ").append(catDao.getIdGender("male"));
                }
            }
        }
        ...
        if (priceRange != null && !priceRange.isEmpty()) {
            switch (priceRange) {
                case "0-100000":
                    sql.append(" AND p.price < 100000 ");
                    break;
                case "200000-300000":
                    sql.append(" AND p.price BETWEEN 200000 AND 300000 ");
                    break;
                case "300000-400000":
                    sql.append(" AND p.price BETWEEN 300000 AND 400000 ");
                    break;
                case "400000-500000":
                    sql.append(" AND p.price BETWEEN 400000 AND 500000 ");
                    break;
                case "500000+":
        ...
        if ("name_asc".equals(sort2)) {
            sql.append(" ORDER BY p.name ASC ");
        } else if ("name_desc".equals(sort2)) {
            sql.append(" ORDER BY p.name DESC ");
        } else if ("BestSeller".equals(sort1)) {
            sql.append(" ORDER BY s.sold DESC ");
        } else {
            sql.append(" ORDER BY p.product_id DESC ");
        }

        try (PreparedStatement st = connection.prepareStatement(sql.toString()); ResultSet rs = st.executeQuery()) {
            while (rs.next()) {
                list.add(mapRow(rs));
            }
        } catch (Exception e) {
            System.out.println("Error ProductDAO.getPublicProducts: " + e.getMessage());
        }
        return list;
    }
