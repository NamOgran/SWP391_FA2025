...
//int id, int quantity, int price, int categoryID, int promoID, String name, String description, String picURL
    ...
    String sql = "SELECT p.product_id, p.name, p.quantity, p.description, p.pic_url, p.price, "
               + "c.type, c.gender, "
               + "pr.promo_percent, pr.start_date, pr.end_date "
               + "FROM product p "
               + "LEFT JOIN category c ON p.category_id = c.category_id "
               + "LEFT JOIN promo pr ON p.promo_id = pr.promo_id "
    ...
    try (PreparedStatement st = connection.prepareStatement(sql);
        ...
        while (rs.next()) {
            product p = new product(
                ...
                rs.getInt("quantity"),
                rs.getInt("price"),
                0, // categoryID không cần cho hiển thị
                0, // promoID không cần cho hiển thị
                rs.getString("name"),
                rs.getString("description"),
                rs.getString("pic_url")
            );

            // ✅ Gắn thêm dữ liệu hiển thị phụ
            String categoryName = rs.getString("type") + " (" + rs.getString("gender") + ")";
            String promoInfo = rs.getInt("promo_percent") + "% (" 
                             + rs.getDate("start_date") + " -> " 
                             + rs.getDate("end_date") + ")";

            p.setCategoryName(categoryName);
            p.setPromoInfo(promoInfo);
    ...
    public boolean update(product p) {
        String sql = "UPDATE product SET name=?, quantity=?, description=?, pic_url=?, price=?, category_id=?, promo_id=? WHERE product_id=?";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
