...
import jakarta.servlet.annotation.MultipartConfig; // [NEW]
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import jakarta.servlet.http.HttpSession;
import jakarta.servlet.http.Part; // [NEW]
...
public class AdminController extends HttpServlet {
    ...
    // [UPDATED] Xử lý upload file ảnh vào cả Target và Source
    private String processUploadFile(HttpServletRequest request) {
        try {
            ...
            if (filePart != null && filePart.getSize() > 0) {
                ...
                // 1. LƯU VÀO THƯ MỤC SERVER (TARGET/BUILD) - Để hiển thị ngay lập tức
                ...
                filePart.write(targetFilePath); // Ghi file vào folder target
                
                // 2. LƯU VÀO THƯ MỤC SOURCE CODE (SRC) - Để giữ file không bị mất khi build lại
                // Logic: Từ folder target, đi ngược ra ngoài để tìm folder src
                // Cấu trúc thường gặp: Project/target/Project-1.0/ -> cần đi ra 2 cấp để về Project/
                File deployDir = new File(applicationPath);
                File targetDir = deployDir.getParentFile(); // folder target
                File projectRoot = targetDir.getParentFile(); // folder Project gốc
                
                // Tạo đường dẫn tới src/main/webapp/images
                ...
                // Chỉ copy nếu thư mục src tồn tại (tránh lỗi khi deploy lên server thật linux không có src)
                if (srcDir.exists()) {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (tab) {
            case "product": {
                ...
                Map<Integer, String> voucherProductMap = new HashMap<>();
                List<Voucher> allVouchers = voucherDAO.getAll();
                for (Voucher p : allVouchers) {
                    if (p.getVoucherPercent() == 0) voucherProductMap.put(p.getVoucherID(), "None");
                    else voucherProductMap.put(p.getVoucherID(), p.getVoucherPercent() + "%");
                }
            ...
            case "staff_account": {
                ...
                List<Staff> paginatedStaff = (totalAccounts > 0 && startIndex < endIndex) 
                        ? filteredStaff.subList(startIndex, endIndex) 
            ...
            case "customer_manage": {
                ...
                // [FIX] SẮP XẾP A-Z THEO USERNAME
                // Nếu muốn sắp xếp theo Tên thật thì đổi c1.getUsername() thành c1.getFullName()
            ...
            case "voucher": {
                String voucherSearch = request.getParameter("voucher_search");
                int voucherPageIndex = parseIntSafe(request.getParameter("voucher_page"), 1);
                ...
                request.setAttribute("voucherTotalPages", voucherTotalPages);
                request.setAttribute("voucherCurrentPage", voucherPageIndex);
                request.setAttribute("voucherSearch", voucherSearch);
                forwardPage = "/admin_VoucherManagement.jsp";
                break;
            }

            case "order": {
                // [UPDATE 3] DAO: Logic sắp xếp đã bỏ Processing
                List<Orders> orderList = orderDAO.getAllOrdersSort();
                List<OrderDetail> orderDetailList = orderDAO.getAllOrdersDetail();

                // [UPDATE 2] Customer Info: Lấy FullName thay vì Username
                Map<Integer, String> customerMap = new HashMap<>();
                List<Customer> customers = customerDAO.getAll();
                for (Customer c : customers) {
                    // Nếu fullName null hoặc rỗng thì fallback về username
                    String displayName = (c.getFullName() != null && !c.getFullName().isEmpty()) 
                                         ? c.getFullName() 
                                         : c.getUsername();
                ...
                // Map Voucher (Giữ nguyên)
                Map<Integer, Integer> voucherValMap = new HashMap<>();
                ...
                // [UPDATE 1] Logic Tính Tổng + Phí Ship
                ...
                for (Orders o : orderList) {
                    ...
                    for (OrderDetail d : orderDetailList) {
                        if (d.getOrderID() == o.getOrderID()) {
                            ...
                            int voucherId = productVoucherMap.getOrDefault(pId, 0);
                            int percent = (voucherId > 0) ? voucherValMap.getOrDefault(voucherId, 0) : 0;
                            long finalPrice = price - (price * percent / 100L);
                            orderTotal += (finalPrice * d.getQuantity());
                        }
                    }
                    
                    // Logic Phí Ship: Nếu tổng < 200k thì cộng 20k
                    if (orderTotal < 200000) {
                        orderTotal += 20000;
                    }
                    
                    calculatedTotalMap.put(o.getOrderID(), orderTotal);
                }
            ...
            case "import": {
                ...
                List<Imports> paginatedImports = (startImp < endImp) 
                        ? filteredImports.subList(startImp, endImp) 
                        : new ArrayList<>();
            ...
            default: {
                ...
                if (orderMap != null) {
                    for (int val : orderMap.values()) totalOrdersChart += val;
                }
    ...
    private void handleProductAction(HttpServletRequest request, HttpServletResponse response, String action) throws IOException {
        ...
        try {
            if ("add_product".equals(action)) {
                ...
                int voucherId = 0;
                if (voucherPercent >= 0) {
                    voucherDAO.addIfNotExist(voucherPercent);
    ...
    private void handleAccountAction(HttpServletRequest request, HttpServletResponse response, String action) throws IOException {
        ...
        try {
            if ("add_staff".equals(action)) {
                ...
                responseData.setIsSuccess(isSuccess);
                if (!isSuccess) responseData.setDescription("Update failed. Email or Phone may already exist.");
            } else if ("delete_account".equals(action)) {
                String username = request.getParameter("username");
                boolean isSuccess = staffDAO.delete(username);
                responseData.setIsSuccess(isSuccess);
                if (!isSuccess) responseData.setDescription("Cannot delete. Data constraints exist.");
            }
        } catch (Exception e) {
            responseData.setIsSuccess(false);
            responseData.setDescription("Server Error: " + e.getMessage());
        ...
        try {
            if ("add_customer".equals(action)) {
                ...
                if (customerDAO.isUsernameTaken(username)) {
                    ...
                    responseData.setIsSuccess(isSuccess);
                    if (!isSuccess) responseData.setDescription("Database Insert Failed.");
                }
                ...
                responseData.setIsSuccess(isSuccess);
                if (!isSuccess) responseData.setDescription("Update failed.");
    ...
    private void handleVoucherAction(HttpServletRequest request, HttpServletResponse response, String action) throws IOException {
        ...
        ProductDAO productDAO = new ProductDAO();
        String voucherPage = request.getParameter("voucher_page");
        String voucherSearch = request.getParameter("voucher_search");
        String msg = "";
        ...
        try {
            if ("add".equals(action) || "edit".equals(action)) {
                ...
                if (percent < 1 || percent > 100) {
                    msg = "voucher_invalid_percent";
                    validationError = true;
                } else if (startStr == null || endStr == null) {
                    msg = "voucher_invalid_date";
                    validationError = true;
                } else {
                    ...
                    if (!validationError) {
                        if ("add".equals(action)) {
                            dao.addVoucher(new Voucher(0, percent, start, end));
                            msg = "voucher_added";
                        } else {
                            int id = parseIntSafe(request.getParameter("id"), 0);
        ...
        redirectUrl.append("?tab=voucher&msg=").append(encode(msg));
        if (voucherPage != null && !voucherPage.isEmpty()) redirectUrl.append("&voucher_page=").append(encode(voucherPage));
        if (voucherSearch != null && !voucherSearch.isEmpty()) redirectUrl.append("&voucher_search=").append(encode(voucherSearch));
    ...
    private void handleCreateImport(HttpServletRequest request, HttpServletResponse response, int staffId) throws IOException {
        ...
        try {
            ...
            if (jsonArray.length() == 0) throw new Exception("No items provided");
    ...
    private void handleUpdateImportStatus(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            boolean ok = importDAO.updateStatus(idStr);
