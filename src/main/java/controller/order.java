...
public class Order extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        FeedBackDAO daoFeedback = new FeedBackDAO(); // [MỚI] Khởi tạo DAO Feedback
        ...
        switch (urlPath) {
            ...
            case URL_HISTORY_ORDERS: {
                ...
                if (newStatus2 != null && !newStatus2.trim().isEmpty()) {
                    ...
                    // Key = "OrderID_ProductID", Value = true (đã đánh giá) / false
                    Map<String, Boolean> reviewedMap = new HashMap<>();

                    for (Orders o : ordersUserList) {
                        // Chỉ kiểm tra đơn đã giao thành công
    ...
    private static int findVoucherPercentById(VoucherDAO daoVoucher, int voucherId) {
        List<Voucher> vouchers = daoVoucher.getAll();
        if (vouchers == null) {
            return 0;
        }
        for (Voucher pr : vouchers) {
            if (pr != null && pr.getVoucherID() == voucherId) {
    ...
    private static float calcUnitPriceWithVoucherSafe(ProductDAO productDAO, VoucherDAO voucherDAO, int productId) {
        ...
        int percent = (p.getVoucherID() > 0) ? findVoucherPercentById(voucherDAO, p.getVoucherID()) : 0;
