...
<html lang="en">
    <head>
        ...
        <style>
            ...
            .grid-template {
                display: grid;
                grid-template-columns: 40px 100px 1fr 120px 130px 140px 50px;
                gap: 15px;
                align-items: center;
            }
            ...
            .cart-item-card:hover { border-color: rgba(160, 129, 108, 0.3); }
            ...
            .size-wrapper { display: flex; align-items: center; }
            .size-label { font-size: 0.8rem; color: #999; margin-right: 5px; }
            .size-select {
                padding: 2px 8px;
            ...
            .size-select:focus { border-color: var(--primary-color); }

            .col-unit-price { text-align: center; color: #666; font-size: 0.95rem; }
            ...
            .qty-btn:hover:not(:disabled) { background: #f8f8f8; color: var(--primary-color); }
            .qty-btn:first-child { border-right: 1px solid #ddd; }
            .qty-btn:last-child { border-left: 1px solid #ddd; }
            ...
            .old-price { font-size: 0.85rem; color: #999; text-decoration: line-through; }
            ...
            .cart-warning { font-size: 0.75rem; margin-top: 4px; font-weight: 500; }
            .cart-warning.error { color: #dc3545; }
            .cart-warning.info { color: #fd7e14; }
            ...
            .text-highlight { color: var(--primary-color) !important; font-size: 1.4rem; }
            ...
            .checkout-btn:hover:not(:disabled) { background-color: var(--primary-dark); opacity: 0.9; }
            ...
            .cart-img-wrapper { border-radius: var(--radius-control); overflow: hidden; }
            .qty-control,
            .size-select,
            .checkout-btn,
            .btn-continue {
            ...
            @media (max-width: 768px) {
                ...
                .cart-item-card { display: block; position: relative; padding: 15px; }
                .grid-template { display: flex; flex-wrap: wrap; gap: 0; }

                .col-check { width: 40px; margin-right: 10px; margin-bottom: 10px; }
                ...
                .cart-info { width: calc(100% - 160px); margin-bottom: 10px; }
                .col-unit-price { display: none; }
                .col-qty { width: auto; margin-right: auto; }
                .col-total { width: auto; margin-left: auto; text-align: right; }

                .col-action { position: absolute; top: 10px; right: 10px; }
                .checkout-steps { display: none; }
            }

            /* === WARNING SPAN FULL ROW (DESKTOP GRID) === */
            .cart-warning-row{
                grid-column: 3 / -1;
                text-align: left;
                margin-top: 6px;
            }
            .cart-warning-row .cart-warning{
                margin-top: 0;
            }

            /* === MOBILE (khi .grid-template chuyển sang flex) === */
            @media (max-width: 768px) {
                .cart-warning-row{
                    flex: 0 0 100%;
                    order: 20;
    ...
    <body>
                    ...
                    <div class="row g-4">
                        <div class="col-lg-8">
                            ...
                            <div class="cart-header grid-template d-none d-md-grid">
                                <div class="col-check">
                                    <input class="form-check-input" type="checkbox" id="selectAll" title="Select all available items">
                                </div>
                            ...
                            <div class="cart-items-column">
                                <c:forEach items="${requestScope.cartList}" var="cart">
                                    ...
                                    <c:set var="isSelectable" value="${not (isMissingProduct or isOutOfStock or isInsufficient or isStoppedSelling)}" />
                                    ...
                                    <div class="cart-item-card grid-template ${isMissingProduct or isOutOfStock or isStoppedSelling ? 'item-disabled' : ''}"
                                        ...
                                        <div class="col-check">
                                            <input class="form-check-input item-check"
                                                   type="checkbox"
                                                   id="ck${cart.productID}${safeSize}"
                                                   data-id="${cart.productID}"
                                                   data-size="${cart.size_name}"
                                                   data-stock="${availableQty}"
                                                   data-qty-id="quantity${cart.productID}${safeSize}"
                                                   data-locked="${isMissingProduct or isOutOfStock or isStoppedSelling}"
                                                   data-auto-recheck="${isInsufficient}"
                                                   <c:if test="${not isSelectable}">disabled</c:if>
                                                   <c:if test="${isSelectable}">checked</c:if>>
                                        </div>
                                        ...
                                        <!-- WARNING ROW (SPAN NGANG) -->
                                        <div class="cart-warning-row">
                                            <c:choose>
                                                <c:when test="${isMissingProduct}">
                                                    <div class="cart-warning error">This product is no longer available. Please remove it from your cart.</div>
                                                </c:when>
                                                <c:when test="${isStoppedSelling}">
                                                    <div class="cart-warning error">This product is no longer for sale. Please remove it from your cart.</div>
                                                </c:when>
                                                <c:when test="${isOutOfStock}">
                                                    <div class="cart-warning error">This item is out of stock. Please remove it from your cart.</div>
                                                </c:when>
                                                <c:when test="${isInsufficient}">
                                                    <div class="cart-warning info warn-insufficient">
                                                        Only ${availableQty} item(s) left in stock. Please reduce your quantity.
                                                    </div>
                                                </c:when>
                                            </c:choose>
                                        </div>
                        ...
                        <div class="col-lg-4">
                            <div class="summary-card">
                                ...
                                <c:if test="${hasCartError}">
                                    <div class="cart-warning info mt-2 text-center">
                                        Some items are unavailable and cannot be selected for checkout.
                                    </div>
                                </c:if>
                            </div>
                        </div>
        ...
        <script>
            function formatVND(amount) { return (amount || 0).toLocaleString('vi-VN') + "\u00A0"; }
            function onlyDigits(s) { return (s || '').toString().replace(/[^\d]/g, ''); }
            function toInt(text) { return parseInt(onlyDigits(text), 10) || 0; }

            function getSelectableChecks() {
                return Array.from(document.querySelectorAll('.item-check')).filter(cb => !cb.disabled);
            }
            ...
            function syncSelectAll() {
                ...
                const selectable = getSelectableChecks();
                const checked = selectable.filter(cb => cb.checked);
                ...
                selectAll.checked = (checked.length === selectable.length);
                selectAll.indeterminate = (checked.length > 0 && checked.length < selectable.length);
            }
            function updateCheckoutState() {
                const btn = document.getElementById('checkoutBtn');
                const selectedCountEl = document.getElementById('selectedCount'); // có thể không tồn tại nữa
                const checked = getCheckedChecks().length;
            ...
            function normalizeSelectionState() {
                document.querySelectorAll('.cart-item-card').forEach(card => {
                    ...
                    const locked = isTrue(cb.getAttribute('data-locked'));
                    const stock = parseInt(cb.getAttribute('data-stock') || '0', 10) || 0;
                    ...
                    const warn = card.querySelector('.warn-insufficient');
                    if (warn) warn.style.display = (qty > stock) ? '' : 'none';

                    if (locked) {
                        cb.checked = false;
                        cb.disabled = true;
                        cb.dataset.autoRecheck = 'false';
                        return;
                    }

                    const shouldEnable = (stock > 0 && qty > 0 && qty <= stock);
                    const autoRecheck = isTrue(cb.dataset.autoRecheck ?? cb.getAttribute('data-auto-recheck'));

                    if (shouldEnable) {
                        const wasDisabled = cb.disabled;
                        cb.disabled = false;

                        if (wasDisabled && autoRecheck) cb.checked = true;
                        cb.dataset.autoRecheck = 'false';
                    } else {
                        if (!cb.disabled) cb.dataset.autoRecheck = cb.checked ? 'true' : 'false';
                        cb.checked = false;
                        cb.disabled = true;
                    }
                });
            ...
            function calcSubtotalFromDom() {
                ...
                document.querySelectorAll('.cart-item-card').forEach(itemCard => {
                    const cb = itemCard.querySelector('.item-check');
            ...
            function recalcTotals() {
                ...
                const discount = toInt(document.getElementById('discountInput') ? document.getElementById('discountInput').value : 0);
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (temp === 0) {
                            ...
                            const holder = document.getElementById('price2' + productID + safe);
                            if (holder) {
                                holder.textContent = (isNaN(line) || line <= 0)
                                        ? formatVND(price * newQty)
                                        : formatVND(line);
                            }
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (selectAll) {
                    selectAll.addEventListener('change', function () {
                        const selectable = getSelectableChecks();
                        selectable.forEach(cb => cb.checked = selectAll.checked);
                        recalcTotals();
                    });
                }
                ...
                if (checkoutForm) {
                    checkoutForm.addEventListener('submit', function (e) {
                        ...
                        recalcTotals();
                        rebuildSelectedInputsForCheckout();
                    });
                }
