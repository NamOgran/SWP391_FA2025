...
// 1. Sửa hàm đếm tổng số lượng (có thêm lọc ngày)
public int getTotalVoucherCount(String searchKeyword, String startDate, String endDate) {
    StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM voucher WHERE 1=1");

    // Lọc theo từ khóa
    if (searchKeyword != null && !searchKeyword.trim().isEmpty()) {
        sql.append(" AND (voucher_id LIKE ? OR CAST(voucher_percent AS VARCHAR) LIKE ?)");
    }
    // Lọc theo ngày bắt đầu
    if (startDate != null && !startDate.isEmpty()) {
        sql.append(" AND start_date >= CAST(? AS DATE)");
    }
    // Lọc theo ngày kết thúc
    if (endDate != null && !endDate.isEmpty()) {
        sql.append(" AND end_date <= CAST(? AS DATE)");
    }
    ...
    try (PreparedStatement st = connection.prepareStatement(sql.toString())) {
        ...
        if (searchKeyword != null && !searchKeyword.trim().isEmpty()) {
            st.setString(index++, "%" + searchKeyword + "%");
            st.setString(index++, "%" + searchKeyword + "%");
        }
        if (startDate != null && !startDate.isEmpty()) {
            st.setString(index++, startDate);
        }
        if (endDate != null && !endDate.isEmpty()) {
            st.setString(index++, endDate);
        }
...
// 2. Sửa hàm lấy danh sách phân trang (có thêm lọc ngày)
public List<Voucher> getPaginatedVouchers(String searchKeyword, String startDate, String endDate, int pageIndex, int pageSize) {
    List<Voucher> list = new ArrayList<>();
